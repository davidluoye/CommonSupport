...
/**
 * Translates {@link RecyclerView.OnScrollListener} events to {@link ViewPager.OnPageChangeCallback} events
 * for {@link ViewPager}. As part of this process, it keeps track of the current scroll position
...
final class ScrollEventAdapter extends RecyclerView.OnScrollListener {
    ...
    private ViewPager.OnPageChangeCallback mCallback;
    ...
    private @ViewPager.ScrollState int mScrollState;
    ...
    ScrollEventAdapter(@NonNull ViewPager viewPager) {
    ...
    public void onScrollStateChanged(@NonNull RecyclerView recyclerView, int newState) {
        ...
        if (isInAnyDraggingState() && newState == RecyclerView.SCROLL_STATE_SETTLING) {
            ...
            if (mScrollHappened) {
                dispatchStateChanged(ViewPager.SCROLL_STATE_SETTLING);
        ...
        if (isInAnyDraggingState() && newState == RecyclerView.SCROLL_STATE_IDLE) {
            ...
            if (dispatchIdle) {
                ...
                dispatchStateChanged(ViewPager.SCROLL_STATE_IDLE);
                resetState();
            }
        }
    ...
    public void onScrolled(@NonNull RecyclerView recyclerView, int dx, int dy) {
        ...
        if ((mScrollValues.mPosition == mTarget || mTarget == NO_POSITION)
                && mScrollValues.mOffsetPx == 0 && !(mScrollState == ViewPager.SCROLL_STATE_DRAGGING)) {
    ...
    private void updateScrollEventValues() {
        ...
        boolean isHorizontal = mLayoutManager.getOrientation() == ViewPager.ORIENTATION_HORIZONTAL;
        int start, sizePx;
        if (isHorizontal) {
    ...
    private void startDrag(boolean isFakeDrag) {
        ...
        dispatchStateChanged(ViewPager.SCROLL_STATE_DRAGGING);
    }
    ...
    void setOnPageChangeCallback(ViewPager.OnPageChangeCallback callback) {
        mCallback = callback;
    }
     ...
     *         {@link ViewPager#endFakeDrag()} is called.
     */
     ...
     * the ViewPager is settling towards its {@link ViewPager#getCurrentItem() current item}, or
    ...
    private void dispatchStateChanged(@ViewPager.ScrollState int state) {
